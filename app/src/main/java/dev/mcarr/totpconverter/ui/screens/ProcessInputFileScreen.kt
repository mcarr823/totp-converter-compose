...
fun ProcessInputFileScreen(
    ...
    val numberParsed by remember {
        mutableStateOf(model.numberParsed)
    }
    ...
    Column(
        ...
        if (success) {
            ...
            PaddedText(text = "${numberParsed} token(s) parsed successfully")
            Spacer(modifier = Modifier.height(16.dp))
            Button(onClick = next) {
                Text(text = "Next")
            }
            ...
            Spacer(modifier = Modifier.height(16.dp))
            PaddedText(text = "Parsing the input file...")
            Spacer(modifier = Modifier.height(16.dp))
...
@Preview
@Composable
fun PreviewProcessInputFileScreen(){
